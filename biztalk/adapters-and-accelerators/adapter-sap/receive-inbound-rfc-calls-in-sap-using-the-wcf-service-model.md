...
-   An RFC destination for the [!INCLUDE[adaptersap_short](../../includes/adaptersap-short-md.md)] must exist. The [!INCLUDE[adaptersap_short](../../includes/adaptersap-short-md.md)] registers itself with an RFC destination to receive RFCs from the SAP system. You supply parameters in the SAP connection URI such as the SAP Gateway Host, the SAP Gateway Service, and the SAP Program ID that the adapter uses to register itself. For information about how to create an RFC destination, see [Creating an RFC Destination in an SAP System](../../adapters-and-accelerators/adapter-sap/creating-an-rfc-destination-in-an-sap-system.md).  
