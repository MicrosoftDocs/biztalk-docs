---
description: "Learn more about: Moving BizTalk Server Databases"
title: "Moving BizTalk Server Databases"
ms.custom: ""
ms.date: "06/08/2017"
ms.service: biztalk-server
ms.reviewer: ""
ms.suite: ""
ms.topic: concept-article
---
# Moving BizTalk Server Databases
In any active data center, databases sometimes need to be moved to different hardware. For example, you might need to move a database when the database outgrows its hardware or when the hardware is upgraded or replaced by newer equipment. The procedures in this section describe how to move the most commonly moved BizTalk Server databases.  
  
## In This Section  
  
-   [How to Move the BizTalk Server Databases](../core/how-to-move-the-biztalk-server-databases.md)  
  
-   [How to Move the BAM Primary Import Database](../core/how-to-move-the-bam-primary-import-database1.md)  
  
-   [How to Move the BAM Archive Database](../core/how-to-move-the-bam-archive-database2.md)  
  
-   [How to Move the BAM Star Schema Database](../core/how-to-move-the-bam-star-schema-database1.md)  
  
-   [How to Move the BAM Analysis Database](../core/how-to-move-the-bam-analysis-database2.md)  
  
-   [How to Move the BAM Notification Services Databases](../core/how-to-move-the-bam-notification-services-databases2.md)
