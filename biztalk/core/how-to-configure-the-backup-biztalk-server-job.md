...
Lists the steps necessary to configure the Backup BizTalk Server job.  

## Overview
After you install and configure BizTalk Server, configure the Backup [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] job to back up your data. The **Backup BizTalk Server (BizTalkMgmtDb)** job includes the following steps:
...
5.  Select the **BackupFull** step, and select **Edit**. In the **Command** box, update the parameter values:  
    ...
    5. Optional. **Local time hour for the backup process to run**: The default is NULL. The backup job is not associated with the time zone of the [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] computer, and runs at midnight UTC time (0000). If you want to backup at a specific hour in the time zone of the [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] computer, enter an integer value from 0 (midnight) to 23 (11 PM) as the local time hour for the **BackupHour** parameter. 
