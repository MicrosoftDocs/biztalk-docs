...
You can deploy policies programmatically by using the [Microsoft.RuleEngine.RuleSetDeploymentDriver](/previous-versions/) class in the **Microsoft.RuleEngine.RuleEngineExtensions** namespace. The following sample code demonstrates how to use the [Microsoft.RuleEngine.RuleSetDeploymentDriver](/previous-versions/) class to deploy a policy named **LoanProcessing**:  
...
> [!NOTE]
>  The overloaded constructors of the [Microsoft.RuleEngine.RuleSetDeploymentDriver](/previous-versions/) class take the names of the rule store database as a parameter. This allows you to deploy policies to a database that your BizTalk Server environment is not configured to use.  
