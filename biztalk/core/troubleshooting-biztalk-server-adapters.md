---
description: "Learn more about: Troubleshooting BizTalk Server Adapters"
title: "Troubleshooting BizTalk Server Adapters"
ms.custom: ""
ms.date: "06/08/2017"
ms.prod: "biztalk-server"
ms.reviewer: ""
ms.suite: ""
ms.topic: "article"
---
# Troubleshooting BizTalk Server Adapters
This topic describes common trouble shooting steps that can be used to identify and resolve problems that may be occurring in the BizTalk Integrated adapters.  
  
## In This Section  
  
-   [Troubleshooting the File Adapter](../core/troubleshooting-the-file-adapter.md)  
  
-   [Troubleshooting the FTP Adapter](../core/troubleshooting-the-ftp-adapter.md)  
  
-   [Troubleshooting the HTTP Adapter](../core/troubleshooting-the-http-adapter.md)  
  
-   [Troubleshooting the MQSeries Adapter](../core/troubleshooting-the-mqseries-adapter.md)  
  
-   [Troubleshooting the MSMQ Adapter](../core/troubleshooting-the-msmq-adapter.md)  
  
-   [Troubleshooting the POP3 Adapter](../core/troubleshooting-the-pop3-adapter.md)  
  
-   [Troubleshooting the SMTP Adapter](../core/troubleshooting-the-smtp-adapter.md)  
  
-   [Troubleshooting the SOAP Adapter](../core/troubleshooting-the-soap-adapter.md)  
  
-   [Troubleshooting the Windows SharePoint Services Adapter](../core/troubleshooting-the-windows-sharepoint-services-adapter.md)  
  
-   [Troubleshooting the WCF Adapters](../core/troubleshooting-the-wcf-adapters.md)  
  
-   [Using BizTalk Adapter Tracing](../core/using-biztalk-adapter-tracing.md)  
  
-   [Using IPv6 Addressing with BizTalk Adapters](../core/using-ipv6-addressing-with-biztalk-adapters.md)
