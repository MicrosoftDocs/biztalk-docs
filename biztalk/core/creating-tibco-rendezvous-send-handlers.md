---
title: "Creating TIBCO Rendezvous Send Handlers | Microsoft Docs"
ms.custom: ""
ms.date: "06/08/2017"
ms.prod: "biztalk-server"
ms.reviewer: ""
 ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
helpviewer_keywords: 
  - "send handlers"
ms.assetid: ad996c4f-e6ed-4582-a768-0cb1ad25b1d8
caps.latest.revision: 5
author: "MandiOhlinger"
ms.author: "mandia"
manager: "anneta"
---
# Creating TIBCO Rendezvous Send Handlers
This section explains how to create a schema to use TIBCO Rendezvous in a BizTalk Server orchestration.  
  
## In This Section  
  
-   [Management](../core/management.md)  
  
-   [How to Create Send Ports for TIBCO Rendezvous](../core/how-to-create-send-ports-for-tibco-rendezvous.md)  
  
-   [How to Set TIBCO Rendezvous Transport Properties](../core/how-to-set-tibco-rendezvous-transport-properties.md)  
  
-   [How to Set Send Pipelines for TIBCO Rendezvous](../core/how-to-set-send-pipelines-for-tibco-rendezvous.md)  
  
-   [Using TIBCO Rendezvous Send Ports from BizTalk Server](../core/using-tibco-rendezvous-send-ports-from-biztalk-server.md)  
  
-   [Using BizTalk Server from TIBCO Rendezvous to Send Messages](../core/using-biztalk-server-from-tibco-rendezvous-to-send-messages.md)  
  
-   [Data Type Mapping for Send Handlers in TIBCO Rendezvous](../core/data-type-mapping-for-send-handlers-in-tibco-rendezvous.md)  
  
-   [BizTalk Server Message Context Properties (Send Handlers)](../core/biztalk-server-message-context-properties-send-handlers.md)