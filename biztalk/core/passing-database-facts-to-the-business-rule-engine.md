...
> [!NOTE]
>  You need to use a [SqlTransaction](https://msdn.microsoft.com/library/system.data.sqlclient.sqltransaction.aspx) object only if the policy updates the database.  
...
2.  Implement the [IFactRemover](https://msdn.microsoft.com/library/Microsoft.RuleEngine.IFactRemover.aspx) interface on fact retriever component, and commit the database changes from the [UpdateFactsAfterExecution](https://msdn.microsoft.com/library/microsoft.ruleengine.ifactremover.updatefactsafterexecution.aspx) method. This method is called by the rule engine after it is done with executing the policy.  
