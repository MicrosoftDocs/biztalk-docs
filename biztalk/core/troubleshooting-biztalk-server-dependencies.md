---
description: "Learn more about: Troubleshooting BizTalk Server Dependencies"
title: "Troubleshooting BizTalk Server Dependencies | Microsoft Docs"
ms.custom: ""
ms.date: "06/08/2017"
ms.prod: "biztalk-server"
ms.reviewer: ""

ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: 677b7ef3-9a91-4f1e-bfc5-926bfab23a0f
caps.latest.revision: 15
author: "MandiOhlinger"
ms.author: "mandia"
manager: "anneta"
---
# Troubleshooting BizTalk Server Dependencies
Microsoft [!INCLUDE[btsBizTalkServerNoVersion](../includes/btsbiztalkservernoversion-md.md)] makes extensive use of several other Microsoft products for design time and run-time operations. This section contains troubleshooting guidelines for resolving problems that can occur in these underlying technologies.  
  
## In This Section  
  
-   [Troubleshooting Problems with MSDTC](../core/troubleshooting-problems-with-msdtc.md)  
  
-   [Troubleshooting Certificates](../core/troubleshooting-certificates.md)  
  
-   [Troubleshooting Enterprise Single Sign-On](../core/troubleshooting-enterprise-single-sign-on.md)  
  
-   [Troubleshooting SQL Server](../core/troubleshooting-sql-server.md)  
  
-   [Troubleshooting Web Services](../core/troubleshooting-web-services.md)  
  
-   [Troubleshooting a Windows Server Cluster](../core/troubleshooting-a-windows-server-cluster.md)  
  
-   [Troubleshooting Internet Information Services](../core/troubleshooting-internet-information-services.md)  
  
-   [Troubleshooting Windows SharePoint Services](../core/troubleshooting-windows-sharepoint-services.md)
