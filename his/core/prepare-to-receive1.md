---
title: "PREPARE_TO_RECEIVE1 | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
...
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: f71c417e-620e-4592-852c-878b445745d2
 ...
 *locks*  
 ...
 Use this parameter only if **ptr_type** is set to AP_SYNC_LEVEL and the synchronization level of the conversation, established by [ALLOCATE](../core/allocate1.md), is AP_CONFIRM_SYNC_LEVEL. (Otherwise, the parameter is ignored.)  
...
-   The partner TP has encountered an ABEND, causing the partner LU to send a **DEALLOCATE** request.  
 ...
 Primary return code; the partner TP (or partner LU) issued a [SEND_ERROR](../core/send-error1.md) verb with **err_type** set to AP_SVC while in RECEIVE, PENDING_POST, CONFIRM, CONFIRM_SEND, or CONFIRM_DEALLOCATE state. Data sent to the partner TP may have been purged.  
  
## Remarks  
 Before changing the conversation state, this verb performs the equivalent of one of the following:  
  
-   [FLUSH](../core/flush1.md), by sending the contents of the local LU's send buffer to the partner LU (and TP).  
  
-   [CONFIRM](../core/confirm1.md), by sending the contents of the local LU's send buffer and a confirmation request to the partner TP.  
...
-   AP_CONFIRM_SEND and replies with [CONFIRMED](../core/confirmed2.md)  
  
-   AP_DATA_COMPLETE_CONFIRM_SEND and replies with **CONFIRMED**  
  
-   AP_DATA_CONFIRM_SEND and replies with **CONFIRMED**  
  
 The receive verbs are [RECEIVE_AND_POST](../core/receive-and-post2.md), [RECEIVE_IMMEDIATE](../core/receive-immediate2.md), and [RECEIVE_AND_WAIT](../core/receive-and-wait1.md).