...
2.  If the token is `KEY`:  
    ...
    3.  If the token is `=` or `EQUALS`, then it should be followed by values. Refer to [Value Parsing](../core/value-parsing.md) for details about the values.  
...
3.  If the token is `POSITION`:  
    ...
    2.  If the token is `=` or `EQUALS`, then it should be followed by values. Refer to [Value Parsing](../core/value-parsing.md) for details about the values.  
