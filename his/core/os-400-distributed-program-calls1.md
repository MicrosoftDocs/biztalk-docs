---
description: "Learn more about: IBM System i Distributed Program Calls"
...
The IBM System i Remote Command and Distributed Program Calls (DPC) programming model allows most IBM System i applications to interact with TI in request-reply fashion (client-initiated only) with minimum modifications. DPC is a documented protocol that supports program to program integration on an IBM System i, which can be accessed easily from PC based applications using the TCP/IP networking protocol.  
  
> [!NOTE]
>  This interface does not support host-initiated processing (HIP); IBM System i integration is for client-initiated calls only.  
  
 The following figure summarizes the workflow occurring between the client, the default DPC Server, and the IBM System i transaction program. The numbers in parentheses indicate the approximate order in which events occur. A more detailed description of the events follows the figure.  
  
 ![Image that shows the IBM System i model flow.](../core/media/his-ti12.gif "his_ti12")  
IBM System i model flow  
  
## Summary Workflow Diagram for the IBM System i DPC Programming Model  
 The IBM System i DPC programming model works as follows:  
...
3. If the application is COM+ component, the TI Automation proxy:  
   ...
   2. Maps the automation data types to IBM System i RPG data types.  
   ...
   4. Maps the .NET Framework data types to IBM System i RPG data types.  
  
      The TI Automation proxy then:  
  
   5. Calls the conversion routines to convert the application data to IBM System i RPG types.  
  
   6. Builds the parameterized message buffer that represents the RPG PLIST.  
  
   7. Passes the message to the IBM System i DPC transport component.  
  
4. The TI TCP transport sends a connection request to the DPC Server system using the Internet Protocol (IP) address of the IBM System i computer and the port address of the server. The TI TCP transport then waits for a reply.  
  
5. The DPC Server on the IBM System i accepts the session request and issues a receive. The DPC Server then waits for the start server request.  
...
12. The TI Automation proxy receives the reply data and processes the reply. The TI Automation proxy:  
    ...
    3. Maps the IBM System i data types to the automation data.  
  
    4. Calls the conversion routines to convert the IBM System i RPG types to the application data.  
  
       If the application is a .NET assembly, the TI Automation proxy:  
  
    5. Maps the IBM System i data types to the .NET Framework data types.  
  
    6. Calls the conversion routines to convert the IBM System i RPG types to the application data.  
...
13. The TI runtime sends the converted data back to the COM or .NET Framework application that invoked the method.  
    ...
    For information about configuring the mainframe and writing server applications for IBM IBM System ie, see the ILE RPG/400 Programmers Guide Version 4 (IBM Document #SC09-2507-02) and the ILE RPG/400 Reference Version 3 (IBM Document #SC09-2077-01).  
...
## See Also  
 ...
 [IBM System i Security](./as-400-security1.md)   
