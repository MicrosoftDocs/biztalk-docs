...
# Host Integration Server 5250 (IBM System i) Connectivity

In the peer-oriented SNA network model, all computers on the network can communicate directly with each other. Advanced Peer-to-Peer Networking (APPN) enables distributed data processing, defines how components communicate with each other, and determines the level of network-related services that are supplied by each computer in the network. Although peer-oriented SNA networks are usually associated with an IBM System i host system, mainframe systems can also support peer-to-peer networking.  
...
In an APPN network, Host Integration Server computers emulate PU 2.1 devices and can connect to an IBM System i using IP-DLC.
