---
title: "Wizards and Dialog Boxes (TI Project)1 | Microsoft Docs"
ms.custom: ""
ms.date: "10/13/2017"
ms.prod: "host-integration-server"
ms.reviewer: ""
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: cd56b965-c2ec-4ecc-9b25-fd2918642bed
caps.latest.revision: 3
---
# Wizards and Dialog Boxes (TI Project)
## In This Section  
 [New .NET Client Library Wizard](../core/new-net-client-library-wizard.md)  
  
 [New .NET Server Library Wizard](../core/new-net-server-library-wizard.md)  
  
 [Import COBOL Wizard](../core/import-cobol-wizard.md)  
  
 [Import RPG Wizard](../core/import-rpg-wizard.md)  
  
 [Properties (TI Project)](../core/properties-ti-project.md)  
  
 [Name Conflict Dialog Box](../core/name-conflict-dialog-box.md)  
  
 [Array Dimension Dialog Box](../core/array-dimension-dialog-box.md)  
  
 [Map Remote Environment Class Dialog Box](../core/map-remote-environment-class-dialog-box.md)  
  
 [Select Convert Prim Dialog Box](../core/select-convert-prim-dialog-box.md)  
  
## See Also  
 [Working with TI Designer](../core/working-with-ti-designer.md)