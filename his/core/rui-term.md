 ...
 *LUA_PARAMETER_CHECK*  
 ...
 Secondary return code; [RUI_INIT](../core/rui-init.md) has not yet completed successfully for the LU name specified on **RUI_TERM**.  
...
## Remarks  
 This verb can be issued at any time after [RUI_INIT](../core/rui-init.md) has been issued (whether or not it has completed). If any other LUA verb is pending when **RUI_TERM** is issued, no further processing on the pending verb takes place, and it returns with a primary return code of LUA_CANCELED.  
