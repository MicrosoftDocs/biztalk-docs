...
## Remarks  
 Data processed by [SEND_DATA](../core/send-data.md) accumulates in the local LU's send buffer until one of the following happens:  
  
-   The local TP issues **FLUSH** (or other verb that flushes the LU's send buffer).  
  
-   The buffer is full.  
  
 The request generated by [ALLOCATE](../core/allocate.md) is also buffered.  
