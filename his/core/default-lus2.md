...
## IBM i Environment  
 In IBM i environments, the ability to assign default APPC LUs to users or groups is especially useful because it gives the administrator centralized control over these LU assignments. In such environments, for each user or group, assign both a default local APPC LU and a default remote APPC LU. Assigning a default local APPC LU for each user fulfills the normal IBM i procedure of assigning local LUs on a per-user basis. Assigning a default remote APPC LU is equivalent to assigning a default IBM i for the user to connect to, since the remote LU designates the IBM i. By making these assignments, the administrator can centrally control the default IBM i that a 5250 emulator user connects to.  
