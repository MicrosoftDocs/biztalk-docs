---
title: "SNA Communication Tuning2 | Microsoft Docs"
ms.custom: ""
ms.date: "10/13/2017"
ms.prod: "host-integration-server"
ms.reviewer: ""
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: 2923e40f-17ca-4668-9b7a-401f3fc3ee09
caps.latest.revision: 3
author: MandiOhlinger
manager: anneta
---
# SNA Communication Tuning
The topics in this section describe the major factors that affect Transaction Integrator (TI) throughput over the APPC/LU 6.2 SNA transport.  
  
## In This Section  
 [LU 6.2 Contention Winner Limit](../core/lu-6-2-contention-winner-limit.md)  
  
 [Pre-Activation of the LU 6.2 Sessions](../core/pre-activation-of-the-lu-6-2-sessions.md)  
  
 [SNA Link Tuning](../core/sna-link-tuning.md)  
  
 [Host (VTAM, CICS or IMS) Response Time](../core/host-vtam-cics-or-ims-response-time.md)  
  
## See Also  
 [Transaction Integrator Performance Guide](../core/transaction-integrator-performance-guide.md)