 ...
 *tp_id*  
 Supplied parameter. Identifies the local TP. The value of this parameter was returned by [TP_STARTED](../core/tp-started1.md) in the invoking TP or by [RECEIVE_ALLOCATE](../core/receive-allocate2.md) in the invoked TP.  
  
 *conv_id*  
 Supplied parameter. Identifies the conversation established between the two TPs. The value of this parameter is returned by [MC_ALLOCATE](../core/mc-allocate1.md) in the invoking TP or by **RECEIVE_ALLOCATE** in the invoked TP.  
 ...
 *dealloc_type*  
 ...
 AP_SYNC_LEVEL uses the conversation's synchronization level (established by [MC_ALLOCATE](../core/mc-allocate1.md)) to determine how to deallocate the conversation. This value is allowed only if the conversation is in SEND state.  
...
-   [MC_FLUSH](../core/mc-flush2.md), by sending the contents of the local LU's send buffer to the partner LU (and TP).  
  
-   [MC_CONFIRM](../core/mc-confirm1.md), by sending the contents of the local LU's send buffer and a confirmation request to the partner TP.  
...
-   If the session is terminated abnormally due to a DLC outage.  
 ...
 [!INCLUDE[hishostintegrationserver2009](../includes/hishostintegrationserver2009-md.md)] allows TPs to deallocate conversations immediately after sending data by specifying the type parameter on [MC_SEND_DATA](../core/mc-send-data2.md) as AP_SEND_DATA_DEALLOC_\*. However, the **MC_SEND_DATA** verbs do not contain the implied forget callback function. TPs wishing to receive implied forget notification must issue **MC_DEALLOCATE** explicitly.