...
-   The **Close(PLU)** protocol is unconditional. It is not possible for the recipient of a [Close(PLU) Request](../core/close-plu-request1.md) (either the local node or an application) to keep the PLU connection open. The only valid response is **Close(PLU) Response** with the close qualifier as RSPOK.