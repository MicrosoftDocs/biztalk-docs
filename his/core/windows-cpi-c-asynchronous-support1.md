...
-   Waiting for Win32 events.  
 ...
 With the exception of an asynchronous [Receive](../core/receive-cpi-c-1.md) call that can issue certain other calls while pending, a conversation can have only one incomplete operation at any time. For more information about using an asynchronous **Receive** call, see [Using Asynchronous Call Completion](../core/asynchronous-call-completion1.md). In the case of an incomplete operation, the program can issue [Wait_For_Conversation](../core/wait-for-conversation-cpi-c-2.md) to test for its completion or [Cancel_Conversation](../core/cancel-conversation-cpi-c-1.md) to end the conversation and the incomplete operation.  
