 ...
 **lua_rh**  
 Supplied parameter. Contains the SNA request/response header (RH) of the message sent or received. It is set for [RUI_WRITE](../core/rui-write.md) and **SLI_SEND**, and returned by [RUI_READ](../core/rui-read.md) and [RUI_BID](../core/rui-bid.md). For the RH for **SLI_SEND_EX**, all fields except the queued-response indicator (**lua_rh.qri**) and pacing indicator (**lua_rh.pi**) are used.  
...
## Return Codes  
 ...
 When issuing [SLI_RECEIVE_EX](../core/sli-receive-ex.md), set at least one lua_flag1 flow flag.  
 ...
 Secondary return code; the application did not provide user-supplied data required by the verb issued. Note that when **SLI_SEND_EX** is issued for an SNA LUSTAT command, status (in four bytes) is required, and that when [SLI_OPEN](../core/sli-open.md) is issued with secondary initialization, data is required.  
 ...
 The supplied data segment for [SLI_RECEIVE_EX](../core/sli-receive-ex.md) or **SLI_SEND_EX** is not a read/write data segment as required.  
 ...
 **SLI_SEND_EX** sends responses, SNA commands, and data from the Windows LUA application to a host LU.  
 ...
 A session must already be open to issue SLI_SEND_EX for a particular LU-LU session flow. To send data on the SSCP normal flow prior to the completion of [SLI_OPEN](../core/sli-open.md), the session must have been initialized as primary with SSCP access. In addition, the session status must be INIT_COMPLETE.  
