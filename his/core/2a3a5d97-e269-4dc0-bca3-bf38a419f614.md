...
##  <a name="conv"></a> Converting COM Type Libraries  
 Type Libraries (TLBs) are no longer supported in [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)]. You must convert any existing COMTI or TI TLB Type Libraries to TI .NET assemblies and then redeploy them.  
...
|x86|C:\Program Files\Microsoft Host Integration Server 2013\system\|  
 ...
 For more information about the command-line syntax for the TIConverionTool utility, see [Transaction Integrator Conversion Tool](../Topic/Transaction%20Integrator%20Conversion%20Tool2.md).  
...
-   New context-enabled features added in [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)], such as Dynamic REs.  
  
 For more information, see the help topic [Working with TI Designer](../Topic/Working%20with%20TI%20Designer2.md).  
...
##  <a name="upgrade"></a> Upgrading .NET Assemblies  
 ...
 IMS Implicit and IMS Explicit are no longer supported in [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] and will be converted to the IMS Connect programming model when converted using the TIConversionTool.exe. For more information, see [IMS Connect](../Topic/IMS%20Connect2.md).  
 ...
 **Deploying converted assemblies**  
 ...
 Support for the .NET Remoting hosting model was dropped in [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)]. You must deploy .NET assemblies using either the WCF or Direct Call hosting models.  
...
##  <a name="cons"></a> Additional Considerations  
 When a type library or assembly is fully upgraded to [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)], the following two aspects of the old type library may be updated depending on the upgrade process you used.  
  
-   Remote environment class.  
  
-   Default conversion information defined at the library level. The defaults for converting data types are different in .NET and COM. For more information, see [Data Types](../Topic/Data%20Types1.md).  
...
|-----------------|----------------|-------------|-----------------|  
|None. Use existing .NET Object and REs|No changes are made to:<br /><br /> -   RE<br /><br /> -   Ti Object GUID<br /><br /> -   .NET assembly default conversions|-   TI .NET Object and RE is fully functional in [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)].<br /><br /> -   The properties within the type library and RE can no longer be modified.<br /><br /> -   New .NET assemblies created with TI Project cannot be associated with older REs.<br /><br /> -   Existing clients do not need modifications.<br /><br /> -   COM TLBs and NET LU6.2 assemblies will not run and must be converted using TIConverisionTool.exe Utility.|-   Initial in place upgrade to [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] from the previous version.<br /><br /> Old TI .NET Objects and old REs work and do not need to be modified.|  
|Convert TLBs and .NET assemblies using the TIConversionTool Utility|No changes are made to:<br /><br /> -   ClientContext model<br /><br /> -   Does not update the Remote Environment class for HIS 2009 & HIS 2006  supported models|-   Old type 2004 TLBs and .NET assemblies are updated to work with [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)]. The updated .NET assembly can only be associated with new [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] REs. It is recommended that converted HIS 2006 TLBs and .NET assemblies also be associated with new [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] REs.<br /><br /> If you migrate an old type library to a .NET assembly, TI also migrates all Visual Basic version 6.0 Automation data types to Visual Basic .NET data types, including:<br /><br /> -   Integer to Short<br /><br /> -   Long to Integer<br /><br /> -   Currency to Decimal<br /><br /> -   Recordset to DataTable<br /><br /> -   UDT to Structure<br /><br /> Client Applications using the **NewRecordset** function to create disconnected recordsets must modify the code. The **NewRecordset** function is not supported in .NET.<br /><br /> -   Existing COM Clients that use declarative binding must be recompiled because of GUID changes.|-   Migrating HIS 2009 TLBs and .NET LU62 assemblies.<br /><br /> -   Migrating from HIS 2006 or HIS 2004 TLBs and .NET assemblies.<br /><br /> -   A new RE must be created in the Host Integration Server 2009 environment.<br /><br /> -   Using [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] RE features.<br /><br /> -   Moving to the [!INCLUDE[hishostintegrationserver2009](../core/includes/hishostintegrationserver2009-md.md)] supported environment.<br /><br /> -   Programming model stays the same.|  
