 ...
 *rtn_ctl*  
 Supplied parameter. Specifies when the local LU, acting on a session request from the local TP, should return control to the local TP. For information about sessions, see [Transaction Programs Overview](../Topic/Transaction%20Programs%20Overview2.md).  
 ...
 *tp_name*  
 Supplied parameter. Specifies the name of the invoked TP. The value of **tp_name** specified by **MC_ALLOCATE** in the invoking TP must match the value of **tp_name** specified by [RECEIVE_ALLOCATE](../core/receive-allocate.md) in the invoked TP.  
 ...
 **MC_ALLOCATE** establishes a mapped conversation.  
 ...
 Several parameters of **MC_ALLOCATE** are EBCDIC or ASCII strings. A TP can use the CSV [CONVERT](../core/convert.md) to translate a string from one character set to the other.  
  
 To send the **MC_ALLOCATE** request immediately, the invoking TP can issue [MC_FLUSH](../core/mc-flush.md) or [MC_CONFIRM](../core/mc-confirm.md) immediately after **MC_ALLOCATE**. Otherwise, the **MC_ALLOCATE** request accumulates with other data in the local LU's send buffer until the buffer is full.  
