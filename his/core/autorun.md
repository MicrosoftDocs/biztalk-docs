---
title: "Autorun | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
ms.prod: "host-integration-server"
ms.reviewer: ""
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: 35f9e1f5-1253-42ea-b852-3d14a167fbe0
caps.latest.revision: 4
---
# Autorun
**Planning**  
  
 We recommend that you read the Host Integration Server 2010 Installation Guide, which covers the following topics:  
  
-   System requirements  
  
-   Feature installation and configuration prerequisites  
  
-   Supported host systems  
  
-   Product installation and configuration (upgrades and setup options)  
  
-   Installing and configuring HIS and Enterprise Single Sign-on (ESSO)  
  
-   Uninstalling HIS and ESSO  
  
 **Installation**  
  
 Click **Host Integration Server** to close launch the Installation Wizard.  
  
 **Exit**  
  
 Click **Exit** to close the Host Integration Server 2010 Autorun program.  
  
## See Also  
 [Installation Help](../core/installation-help1.md)   
 [Installation Guide &#91;HIS2010&#93;](http://msdn.microsoft.com/en-us/436d05be-f7e3-479d-a2ee-a8ff14722176)