 ...
 *rts_rcvd*  
 Returned parameter. Indicates whether the partner TP issued [MC_REQUEST_TO_SEND](../core/mc-request-to-send2.md), which requests the local TP to change the conversation to RECEIVE state.  
  
 To change to RECEIVE state the local TP can use [MC_PREPARE_TO_RECEIVE](../core/mc-prepare-to-receive2.md), [MC_RECEIVE_AND_WAIT](../core/mc-receive-and-wait1.md), or [MC_RECEIVE_AND_POST](../core/mc-receive-and-post1.md).  
...
## Return Codes  
 ...
 Secondary return code; the local TP attempted to use **MC_CONFIRM** in a conversation with a synchronization level of AP_NONE. The synchronization level, established by [MC_ALLOCATE](../core/mc-allocate1.md), must be AP_CONFIRM_SYNC_LEVEL.  
...
## Remarks  
 In response to **MC_CONFIRM**, the partner TP normally issues [MC_CONFIRMED](../core/mc-confirmed2.md) to confirm that it has received the data without error. (If the partner TP encounters an error, it issues [MC_SEND_ERROR](../core/mc-send-error1.md) or abnormally deallocates the conversation.)  
  
 The TP can issue **MC_CONFIRM** only if the conversation's synchronization level, established by [MC_ALLOCATE](../core/mc-allocate1.md), is AP_CONFIRM_SYNC_LEVEL.  
...
-   [TP_ENDED](../core/tp-ended2.md)  
 ...
 By issuing **MC_CONFIRM** after[MC_ALLOCATE](../core/mc-allocate1.md), the invoking TP can immediately determine whether the allocation was successful (if **synclevel** is set to AP_CONFIRM_SYNC_LEVEL).  
 ...
 Several parameters of **MC_ALLOCATE** are EBCDIC or ASCII strings. A TP can use the common service verb (CSV) [CONVERT](../core/convert1.md) to translate a string from one character set to the other.  
  
 To send the **MC_ALLOCATE** request immediately, the invoking TP can issue [MC_FLUSH](../core/mc-flush2.md) or **MC_CONFIRM** immediately after **MC_ALLOCATE**. Otherwise, the **MC_ALLOCATE** request accumulates with other data in the local LU's send buffer until the buffer is full.