...
-   PLU and SLU support the following data flow control (DFC) commands (an asterisk [*] indicates commands permitted by the FM profile that are never sent by [!INCLUDE[hisHostIntServNoVersion](../core/includes/hishostintservnoversion-md.md)]):  
...
-   Normal-flow send/receive mode  
 ...
 The next three tables list the FM Profile 3 options that [!INCLUDE[hisHostIntServNoVersion](../core/includes/hishostintservnoversion-md.md)] supports. Host Integration Server rejects any BIND option that specifies an option value not listed in these tables.  
