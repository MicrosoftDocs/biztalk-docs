...
-   A STATION LPI connection for each active secondary station.  
 ...
 When the stations are activated on a multipoint link, the majority of messages flow across the STATION LPI connections. If a connection to a particular secondary station is to be closed (because the operator deactivates it, for instance), the node issues a [Close(STATION) Request](./close-station-request1.md). The SNALink replies with a [Close(STATION) Response](./close-station-response2.md) to the node and sends a Discontact (DISC) frame to the secondary station.  
