---
title: "Filler1 | Microsoft Docs"
ms.custom: ""
ms.date: "10/13/2017"
ms.prod: "host-integration-server"
ms.reviewer: ""
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: d69c762c-84c3-44bf-afc0-f79e41ed23e4
caps.latest.revision: 3
---
# Filler
This section provides recommendations for handling COBOL-based filler data.  
  
## In This Section  
 [COBOL FILLER](../core/cobol-filler.md)  
  
 [TI Application Cannot Reference FILLER](../core/ti-application-cannot-reference-filler.md)  
  
 [How to Use REDEFINES in COBOL](../core/how-to-use-redefines-in-cobol.md)  
  
 [FILLER Optimization](../core/filler-optimization.md)  
  
 [FILLER for Discontiguous Output Area and Return Value](../core/filler-for-discontiguous-output-area-and-return-value.md)