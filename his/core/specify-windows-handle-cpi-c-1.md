...
## Remarks  
 An application can set the processing mode by calling [Set_Processing_Mode](../core/set-processing-mode-cpi-c-1.md). If the Windows handle is set to NULL, or this call is never issued, the application must call [Wait_For_Conversation](../core/wait-for-conversation-cpi-c-2.md) to be notified when the outstanding operation completes.  
