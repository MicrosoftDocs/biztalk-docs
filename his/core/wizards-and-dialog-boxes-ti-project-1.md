---
title: "Wizards and Dialog Boxes (TI Project)1 | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
ms.prod: "host-integration-server"
ms.reviewer: ""
ms.suite: ""
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: cd56b965-c2ec-4ecc-9b25-fd2918642bed
caps.latest.revision: 3
author: "gplarsen"
ms.author: "hisdocs"
manager: "anneta"
---
# Wizards and Dialog Boxes (TI Project)
## In This Section  
 [New .NET Client Library Wizard](../core/new-net-client-library-wizard1.md)  
  
 [New .NET Server Library Wizard](../core/new-net-server-library-wizard1.md)  
  
 [Import COBOL Wizard](../core/import-cobol-wizard2.md)  
  
 [Import RPG Wizard](../core/import-rpg-wizard2.md)  
  
 [Properties (TI Project)](../core/properties-ti-project-2.md)  
  
 [Name Conflict Dialog Box](../core/name-conflict-dialog-box2.md)  
  
 [Array Dimension Dialog Box](../core/array-dimension-dialog-box2.md)  
  
 [Map Remote Environment Class Dialog Box](../core/map-remote-environment-class-dialog-box1.md)  
  
 [Select Convert Prim Dialog Box](../core/select-convert-prim-dialog-box2.md)  
  
## See Also  
 [Working with TI Designer](../core/working-with-ti-designer1.md)