...
-   AP_SEND indicates, for the partner TP, the conversation has entered RECEIVE state. For the local TP, the conversation is now in SEND state. Upon receiving this value, the local TP normally uses [SEND_DATA](../core/send-data.md)to begin sending data.  
...
## Return Codes  
 ...
 AP_DEALLOC_NORMAL  
 Primary return code; the partner TP has deallocated the conversation without requesting confirmation. The partner TP issued [DEALLOCATE](../core/deallocate.md) with **dealloc_type** set to one of the following:  
...
1.  The local TP issues a receive verb until it finishes receiving a complete unit of data. The data received can be:  
    ...
    -   A buffer of data received independent of its logical-record format.  
     ...
     The receive verbs are [RECEIVE_AND_POST](../core/receive-and-post.md), [RECEIVE_AND_WAIT](../core/receive-and-wait.md), and **RECEIVE_IMMEDIATE**.  
