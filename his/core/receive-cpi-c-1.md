...
## Values returned in the status_received parameter  
 ...
 Upon receiving this value, the local program normally uses [Send_Data](../core/send-data-cpi-c-1.md) to begin sending data.  
  
 CM_CONFIRM_DEALLOC_RECEIVED  
 Indicates that the partner program issued [Deallocate](../core/deallocate-cpi-c-2.md) with confirmation requested. For the local program, the conversation is now in CONFIRM_DEALLOCATE state.  
  
 Upon receiving this value, the local program normally issues the [Confirmed](../core/confirmed-cpi-c-1.md) call.  
  
 CM_CONFIRM_RECEIVED  
 Indicates that the partner program issued the [Confirm](../core/confirm-cpi-c-1.md) call. For the local program, the conversation is in CONFIRM state.  
...
## Values returned in the request_to_send_received parameter  
 ...
 The partner program issued the [Request_To_Send](../core/request-to-send-cpi-c-2.md) call, which requests the local program to change the conversation to RECEIVE state.  
...
## Values returned in the return_code parameter  
 ...
 Primary return code; the conversation has been deallocated normally. The partner program issued [Deallocate](../core/deallocate-cpi-c-2.md) with the conversations deallocate type set to CM_DEALLOCATE_FLUSH or CM_DEALLOCATE_SYNC_LEVEL with the synchronization level of the conversation specified as CM_NONE.  
...
-   In a basic conversation, the conversation is in SEND state, the receive type is set to CM_RECEIVE_AND_WAIT, and the program did not finish sending a logical record.  
 ...
 Primary return code; while in SEND state or in SEND_PENDING state with the error direction set to CM_SEND_ERROR, the partner program issued [Send_Error](../core/send-error-cpi-c-1.md). Data was not truncated.  
...
-   The data_received and status_received parameters.  
 ...
 If no data is currently available and the receive type (set by [Set_Receive_Type](../core/set-receive-type-cpi-c-1.md)) is set to CM_RECEIVE_AND_WAIT, the local program waits for data to arrive. If the receive type is set to CM_RECEIVE_IMMEDIATE, the local program does not wait.  
...
-   The local program determines the next action to take based on the control information received through *status_received*. The local program may have to reissue **Receive** to receive the control information.  
 ...
 The conversation type is set by [Set_Conversation_Type](../core/set-conversation-type-cpi-c-2.md). The fill characteristic is set by [Set_Fill](../core/set-fill-cpi-c-2.md).  
...
-   [Confirmation](../core/confirmation-cpi-c-2.md)  
  
-   [Normal Deallocation](../core/normal-deallocation-cpi-c-2.md)  
  
-   [ABEND](../core/abend-cpi-c-1.md)  
  
-   [Errors](../core/errors-cpi-c-2.md)