 ...
 **lua_max_length**  
 Specifies the length of received buffer for [RUI_READ](../core/rui-read.md)and **SLI_RECEIVE**.  
 ...
 **lua_flag1**  
 ...
 Set **lua_flag1.bid_enable** to 1 to re-enable the most recent [SLI_BID](../core/sli-bid.md) (equivalent to issuing **SLI_BID** again with exactly the same parameters as before), or set it to zero if you do not want to re-enable **SLI_BID**. Note that re-enabling the previous **SLI_BID** reuses the VCB originally allocated for it, so this VCB must not have been freed or modified.  
 ...
 **lua_flag2**  
 Returned parameter. Contains flags for messages returned by LUA. Returned by [RUI_BID](../core/rui-bid.md), [RUI_READ](../core/rui-read.md), **RUI_WRITE**, **SLI_BID**, **SLI_RECEIVE**, and [SLI_SEND](../core/sli-send.md). Its subparameters are as follows:  
...
## Return Codes  
 ...
 Secondary return code; the buffer with the [SLI_BID](../core/sli-bid.md) VCB was released before the **SLI_RECEIVE** with **lua_flag1.bid_enable** set to 1 was issued.  
...
-   When issuing [SLI_SEND](../core/sli-send.md) to send an SNA response, set only one **lua_flag1** flow flag.  
...
-   When issuing SLI_RECEIVE, set at least one lua_flag1 flow flag.  
 ...
 Secondary return code; a session was not open or was down due to an [SLI_CLOSE](../core/sli-close.md) or session failure when a command was issued.  
 ...
 Secondary return code; an unexpected error was posted to the SLI by [RUI_WRITE](../core/rui-write.md).  
 ...
 Secondary return code; no session has been initialized for the LUA verb issued or some verb other than [SLI_OPEN](../core/sli-open.md) was issued before the session was initialized.  
...
-   The supplied data segment for **SLI_RECEIVE** or [SLI_SEND](../core/sli-send.md) is not a read/write data segment as required.  
...
-   An invalid sequence of requests for activation  
 ...
 Secondary return code; [SLI_PURGE](../core/sli-purge.md) was issued and canceled **SLI_RECEIVE**.  
...
-   An SNA UNBIND type 0x02 command was received, which means a new BIND is coming. If the UNBIND type 0x02 is received after the beginning [SLI_OPEN](../core/sli-open.md) is complete, the session is suspended until a BIND, optional CRV and STSN, and SDT flows are received. These routines are re-entrant because they have to be called again. The session resumes after the SLI processes the SDT command. If the UNBIND type 0x02 is received while the **SLI_OPEN** is still processing, the primary return code is session-failure, not status.  
  
-   The receipt of an SNA CLEAR caused the suspension. Receipt of an SNA SDT will cause the session to resume.  
  
 LUA_INIT_COMPLETE  
  
 Secondary return code; the LUA interface initialized the session while [SLI_OPEN](../core/sli-open.md) was processing. LUA applications that issue **SLI_OPEN** with **lua_open_type_prim_sscp** receive this status on **SLI_RECEIVE** or [SLI_BID](../core/sli-bid.md).  
...
## Remarks  
 **SLI_RECEIVE** receives responses, SNA commands, and request unit data from the host. **SLI_RECEIVE** also provides the status of the session to the Windows LUA application. An [SLI_OPEN](../core/sli-open.md) request must complete before **SLI_RECEIVE** can be issued. However, if **SLI_OPEN** is issued with **lua_init_type** set to LUA_INIT_TYPE_PRIM_SSCP, an **SLI_RECEIVE** over the SSCP normal flow can be issued as soon as **SLI_OPEN** returns an IN_PROGRESS.  
...
-   LU normal  
 ...
 If [SLI_BID](../core/sli-bid.md) successfully completes before SLI_RECEIVE is issued, the Windows LUA interface can be instructed to reuse the last SLI_BID verbs VCB. To do this, issue SLI_RECEIVE with lua_flag1.bid_enable set to 1.  
...
-   When an SNA UNBIND type BIND forthcoming causes suspension of the session and the [SLI_OPEN](../core/sli-open.md) that opened the session is completed, the session is suspended until the SLI receives a BIND and SDT command. The session can also optionally receive an STSN command. As a result, user-supplied routines issued with the initial SLI_OPEN must be re-entered because they will be recalled.  
 ...
 LUA_SESSION_END_REQUESTED indicates that the application received an SNA SHUTD from the host. The Windows LUA application should issue [SLI_CLOSE](../core/sli-close.md) to close the session when convenient.  
  
 LUA_INIT_COMPLETE is returned only when lua_init_type for [SLI_OPEN](../core/sli-open.md) is LUA_INIT_TYPE_PRIM_SSCP. The status means that the SLI_OPEN has been processed sufficiently to allow SSCP data to now be sent or received.  
  
## Exception Requests  
 If a host application request unit is converted into an EXR, sense data will be returned. When [SLI_BID](../core/sli-bid.md) completes with the returned verb parameters set as shown, an EXR conversion occurs.  
