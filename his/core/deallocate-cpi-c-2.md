---
title: "Deallocate (CPI-C)2 | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
...
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: 787f3fc0-34c6-4d4a-8d9f-cab268bc918e
 ...
 **State Changes**  
 ...
 Depending on the value of the conversations deallocate type parameter (set by [Set_Deallocate_Type](../core/set-deallocate-type-cpi-c-2.md)), the conversation can be in one of the states indicated in the following table when the program issues **Deallocate**:  
...
## Remarks  
 Before deallocating the conversation, this call performs the equivalent of either the [Flush](../core/flush-cpi-c-1.md) or [Confirmed](../core/confirmed-cpi-c-1.md) call, depending on the current conversation synchronization level and deallocate type. The deallocate type is set by [Set_Deallocate_Type](../core/set-deallocate-type-cpi-c-2.md).  
...
-   *return_code* is CM_DEALLOCATED_ABEND  
 ...
 For a basic conversation, if the conversations deallocate type is set to CM_DEALLOCATE_ABEND and the log data length is greater than zero, the local LU writes the log data (specified by [Set_Log_Data](../core/set-log-data-cpi-c-1.md)) to the local error log and to the partner LU.  
