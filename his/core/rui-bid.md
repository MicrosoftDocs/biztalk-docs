 ...
 *lua_peek_data*  
 The union member of **LUA_SPECIFIC** used by the **RUI_BID** and [SLI_BID](../core/sli-bid.md)verbs. Returned parameter. Contains up to 12 bytes of the data waiting to be read.  
...
## Return Codes  
 ...
 Secondary return code; [RUI_TERM](../core/rui-term.md) was issued while this verb was pending.  
 ...
 Secondary return code; [RUI_INIT](../core/rui-init.md) has not yet completed successfully for the LU name specified on this verb.  
 ...
 LUA_NEGATIVE_RSP  
 Primary return code; LUA detected an error in the data received from the host. Instead of passing the received message to the application on [RUI_READ](../core/rui-read.md), LUA discards the message (and the rest of the chain if it is in a chain), and sends a negative response to the host.  
  
 LUA informs the application on a subsequent **RUI_READ** or **RUI_BID** that a negative response was sent.  
  
 The secondary return code contains the sense code sent to the host on the negative response. For information about interpreting the sense code values that can be returned, see [SNA Considerations Using LUA](../Topic/SNA%20Considerations%20with%20LUA2.md).  
  
 A zero secondary return code indicates that, following a previous [RUI_WRITE](../core/rui-write.md) of a negative response to a message in the middle of a chain, LUA has now received and discarded all messages from this chain.  
...
## Remarks  
 **RUI_BID** is used by applications that require notification that a message is waiting to be read. This allows the application to determine how it will handle the message before issuing [RUI_READ](../core/rui-read.md).  
...
-   [RUI_INIT](../core/rui-init.md) must complete successfully before this verb is issued.  
  
-   Only one **RUI_BID** can be outstanding at any one time.  
  
-   After **RUI_BID** has completed successfully, it can be reissued by setting **lua_flag1.bid_enable** on a subsequent [RUI_READ](../core/rui-read.md). If the verb is reissued in this way, the application must not free or modify the storage associated with the **RUI_BID** record.  
