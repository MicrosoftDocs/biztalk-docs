...
---
# SLI_BID
The **SLI_BID** verb notifies the Session Level Interface (SLI) application that a message is waiting to be read using [SLI_RECEIVE](../HIS2010/sli-receive1.md). **SLI_BID** also provides the current status of the session to the Windows logical unit application (LUA) application.  
 ...
 **lua_data_ptr**  
 Pointer to the application-supplied buffer that contains the data to be sent for [SLI_SEND](../HIS2010/sli-send1.md) and [RUI_WRITE](../HIS2010/rui-write1.md) or that will receive data for [SLI_RECEIVE](../HIS2010/sli-receive1.md) and [RUI_READ](../HIS2010/rui-read1.md). Not used by other RUI and SLI verbs and should be set to zero.  
 ...
 **lua_peek_data**  
 The union member of **LUA_SPECIFIC** used by the [RUI_BID](../HIS2010/rui-bid2.md)and **SLI_BID** verbs. Returned parameter. Contains up to 12 bytes of the data waiting to be read.  
...
## Return Codes  
 ...
 If the UNBIND type 0x02 is received after the beginning [SLI_OPEN](../HIS2010/sli-open1.md) is complete, the session is suspended until a BIND, optional CRV and STSN, and SDT flows are received. These routines are re-entrant because they have to be called again. The session resumes after the SLI processes the SDT command.  
 ...
 Secondary return code; the LUA interface initialized the session while **SLI_OPEN** was processing. LUA applications that issue **SLI_OPEN** with the **lua_open_type_prim_sscp** parameter receive this status on [SLI_RECEIVE](../HIS2010/sli-receive1.md) or **SLI_BID**.  
...
-   Provides a preview of the next message that will be read by [SLI_RECEIVE](../HIS2010/sli-receive1.md).  
