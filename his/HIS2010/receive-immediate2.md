...
---
# RECEIVE_IMMEDIATE
The **RECEIVE_IMMEDIATE** verb receives any data currently available from the partner transaction program (TP). If no data is available, the local TP does not wait. To avoid blocking the conversation, the Microsoft Windows operating system can issue [RECEIVE_AND_WAIT](../HIS2010/receive-and-wait1.md) in conjunction with [WinAsyncAPPC](../HIS2010/winasyncappc2.md).  
...
-   AP_YES indicates that the partner TP has issued [REQUEST_TO_SEND](../HIS2010/request-to-send2.md), which requests that the local TP change the conversation to RECEIVE state.  
...
-   AP_CONFIRM_DEALLOCATE indicates that the partner TP has issued [DEALLOCATE](../HIS2010/deallocate1.md) with **dealloc_type** set to AP_SYNC_LEVEL, and the conversation's synchronization level, established by **ALLOCATE**, is AP_CONFIRM_SYNC_LEVEL. Upon receiving this value, the local TP normally issues [CONFIRMED](../HIS2010/confirmed2.md).  
  
-   AP_CONFIRM_SEND indicates that the partner TP has issued [PREPARE_TO_RECEIVE](../HIS2010/prepare-to-receive1.md) with **ptr_type** set to AP_SYNC_LEVEL, and the conversation's synchronization level, established by **ALLOCATE**, is AP_CONFIRM_SYNC_LEVEL. Upon receiving this value, the local TP normally issues **CONFIRMED**, and begins to send data.  
  
-   AP_CONFIRM_WHAT_RECEIVED indicates that the partner TP has issued [CONFIRM](../HIS2010/confirm1.md). Upon receiving this value, the local TP normally issues **CONFIRMED**.  
...
-   AP_DATA_COMPLETE indicates, for **RECEIVE_IMMEDIATE** with **fill** set to AP_LL in basic conversations, that the local TP has received a complete logical record or the end of a logical record.  
     ...
     If **primary_rc** contains AP_DEALLOC_NORMAL, the conversation has been deallocated in response to [DEALLOCATE](../HIS2010/deallocate1.md) issued by the partner TP.  
...
-   AP_SEND indicates, for the partner TP, the conversation has entered RECEIVE state. For the local TP, the conversation is now in SEND state. Upon receiving this value, the local TP normally uses [SEND_DATA](../HIS2010/send-data2.md)to begin sending data.  
...
## Return Codes  
 ...
 AP_DEALLOC_NORMAL  
 Primary return code; the partner TP has deallocated the conversation without requesting confirmation. The partner TP issued [DEALLOCATE](../HIS2010/deallocate1.md) with **dealloc_type** set to one of the following:  
...
-   The local LU is not configured.  
 ...
 Primary return code; the partner TP has issued [SEND_ERROR](../HIS2010/send-error1.md) with **err_type** set to AP_PROG while the conversation was in SEND state. Data was not truncated.  
...
1.  The local TP issues a receive verb until it finishes receiving a complete unit of data. The data received can be:  
    ...
    -   A buffer of data received independent of its logical-record format.  
     ...
     The receive verbs are [RECEIVE_AND_POST](../HIS2010/receive-and-post2.md), [RECEIVE_AND_WAIT](../HIS2010/receive-and-wait1.md), and **RECEIVE_IMMEDIATE**.  
