...
The **Set_Return_Control** call (function name **cmsrc**) is issued by the invoking program to specify when the local logical unit (LU), acting on the session request from the local programs [Allocate](../HIS2010/allocate-cpi-c-1.md) call, should return control to the local program.  
 ...
 *return_control*  
 Supplied parameter. Specifies when the local LU, acting on the [Allocate](../HIS2010/allocate-cpi-c-1.md) call, should return control to the local program. The following are allowed values:  
...
## Remarks  
 This call overrides the default return control established by [Initialize_Conversation](../HIS2010/initialize-conversation-cpi-c-2.md). By default, control is returned when the session is allocated. This call cannot be issued after the [Allocate](../HIS2010/allocate-cpi-c-1.md) call has been issued.  
