---
title: "RUI_READ1 | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
...
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: f3466ccf-459d-407a-9a51-7e6ede1bb9da
 ...
 *lua_max_length*  
 Specifies the length of received buffer for **RUI_READ** and [SLI_RECEIVE](../HIS2010/sli-receive1.md). Not used by other RUI and SLI verbs and should be set to zero.  
 ...
 *lua_flag1*  
 ...
 Set **lua_flag1.bid_enable** to 1 to re-enable the most recent [RUI_BID](../HIS2010/rui-bid2.md) (equivalent to issuing **RUI_BID** again with exactly the same parameters as before), or set it to zero if you do not want to re-enable **RUI_BID**.  
 ...
 *lua_flag2*  
 ...
 Indicates that [RUI_BID](../HIS2010/rui-bid2.md) was successfully re-enabled if set to 1.  
...
## Return Codes  
 ...
 This is not the default behavior for **RUI_READ** and is only enabled when **lua_resv56[3]** is set to a nonzero value in the verb control block when calling [RUI_INIT](../HIS2010/rui-init2.md) during session establishment. For more details, see Remarks.  
 ...
 Secondary return code; **RUI_READ** has been canceled by [RUI_PURGE](../HIS2010/rui-purge1.md).  
  
 LUA_TERMINATED  
  
 Secondary return code; [RUI_TERM](../HIS2010/rui-term1.md) was issued while **RUI_READ** was pending.  
 ...
 Secondary return code; **lua_flag1.bid_enable** was set to re-enable [RUI_BID](../HIS2010/rui-bid2.md) but the previous **RUI_BID** was still in progress.  
 ...
 Secondary return code; **lua_flag1.bid_enable** was set to re-enable [RUI_BID](../HIS2010/rui-bid2.md), but there was no previous **RUI_BID** that could be enabled. (For more information, see Remarks.)  
...
-   The LUA application previously sent a negative response to a message in the middle of a chain. LUA has purged subsequent messages in this chain, and is now reporting to the application that all messages from the chain have been received and purged.  
 ...
 A secondary return code of zero indicates that, following a previous [RUI_WRITE](../HIS2010/rui-write1.md) of a negative response to a message in the middle of a chain, LUA has now received and discarded all messages from this chain.  
 ...
 Secondary return code; the OS/2 process that issued this verb was not the same process that issued [RUI_INIT](../HIS2010/rui-init2.md) for this session. Only the process that started a session can issue verbs on that session.  
...
## Remarks  
 [RUI_INIT](../HIS2010/rui-init2.md) must have completed successfully before **RUI_READ** is issued.  
...
-   LU normal  
 ...
 If [RUI_BID](../HIS2010/rui-bid2.md) successfully completes before an **RUI_READ** is issued, the Windows LUA interface can be instructed to reuse the last **RUI_BID** verbs VCB. To do this, issue the **RUI_READ** with **lua_flag1.bid_enable** set.  
...
-   No other **RUI_BID** is pending.  
 ...
 After a message has been read using **RUI_READ,** it is removed from the incoming message queue and cannot be accessed again. ([RUI_BID](../HIS2010/rui-bid2.md) can be used as a nondestructive read. The application can use it to check the type of data available, but the data remains on the incoming queue and does not need to be used immediately.)  
