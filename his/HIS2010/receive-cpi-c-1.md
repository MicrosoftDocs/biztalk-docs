---
title: "Receive (CPI-C)1 | Microsoft Docs"
ms.custom: ""
ms.date: "11/30/2017"
...
ms.tgt_pltfrm: ""
ms.topic: "article"
ms.assetid: df7fa53f-fe14-4b17-b374-33d15cd44946
...
## Values returned in the status_received parameter  
 ...
 Upon receiving this value, the local program normally uses [Send_Data](../HIS2010/send-data-cpi-c-1.md) to begin sending data.  
  
 CM_CONFIRM_DEALLOC_RECEIVED  
 Indicates that the partner program issued [Deallocate](../HIS2010/deallocate-cpi-c-2.md) with confirmation requested. For the local program, the conversation is now in CONFIRM_DEALLOCATE state.  
  
 Upon receiving this value, the local program normally issues the [Confirmed](../HIS2010/confirmed-cpi-c-1.md) call.  
  
 CM_CONFIRM_RECEIVED  
 Indicates that the partner program issued the [Confirm](../HIS2010/confirm-cpi-c-1.md) call. For the local program, the conversation is in CONFIRM state.  
...
## Values returned in the request_to_send_received parameter  
 ...
 The partner program issued the [Request_To_Send](../HIS2010/request-to-send-cpi-c-2.md) call, which requests the local program to change the conversation to RECEIVE state.  
...
## Values returned in the return_code parameter  
 ...
 Primary return code; the conversation has been deallocated normally. The partner program issued [Deallocate](../HIS2010/deallocate-cpi-c-2.md) with the conversations deallocate type set to CM_DEALLOCATE_FLUSH or CM_DEALLOCATE_SYNC_LEVEL with the synchronization level of the conversation specified as CM_NONE.  
...
-   In a basic conversation, the conversation is in SEND state, the receive type is set to CM_RECEIVE_AND_WAIT, and the program did not finish sending a logical record.  
 ...
 Primary return code; while in SEND state or in SEND_PENDING state with the error direction set to CM_SEND_ERROR, the partner program issued [Send_Error](../HIS2010/send-error-cpi-c-1.md). Data was not truncated.  
...
-   The partner program did not deallocate the conversation before terminating.  
 ...
 Primary return code; while in SEND state, the partner program or partner LU issued [Send_Error](../HIS2010/send-error-cpi-c-1.md) with the type parameter set to SVC. Data sent to the partner program may have been purged.  
...
-   The data_received and status_received parameters.  
 ...
 If no data is currently available and the receive type (set by [Set_Receive_Type](../HIS2010/set-receive-type-cpi-c-1.md)) is set to CM_RECEIVE_AND_WAIT, the local program waits for data to arrive. If the receive type is set to CM_RECEIVE_IMMEDIATE, the local program does not wait.  
...
-   The local program determines the next action to take based on the control information received through *status_received*. The local program may have to reissue **Receive** to receive the control information.  
 ...
 The conversation type is set by [Set_Conversation_Type](../HIS2010/set-conversation-type-cpi-c-2.md). The fill characteristic is set by [Set_Fill](../HIS2010/set-fill-cpi-c-2.md).  
...
-   [Confirmation](../HIS2010/confirmation-cpi-c-2.md)  
  
-   [Normal Deallocation](../HIS2010/normal-deallocation-cpi-c-2.md)  
  
-   [ABEND](../HIS2010/abend-cpi-c-1.md)  
  
-   [Errors](../HIS2010/errors-cpi-c-2.md)