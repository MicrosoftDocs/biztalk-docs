 ...
 **State Changes**  
 ...
 Depending on the value of the conversations deallocate type parameter (set by [Set_Deallocate_Type](../HIS2010/set-deallocate-type-cpi-c-2.md)), the conversation can be in one of the states indicated in the following table when the program issues **Deallocate**:  
...
-   *return_code* is CM_DEALLOCATED_ABEND  
 ...
 For a basic conversation, if the conversations deallocate type is set to CM_DEALLOCATE_ABEND and the log data length is greater than zero, the local LU writes the log data (specified by [Set_Log_Data](../HIS2010/set-log-data-cpi-c-1.md)) to the local error log and to the partner LU.  
