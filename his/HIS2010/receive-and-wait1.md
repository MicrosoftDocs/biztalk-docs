...
The **RECEIVE_AND_WAIT** verb receives any data that is currently available from the partner transaction program (TP). If no data is currently available, the local TP waits for data to arrive.  
 ...
 To allow full use to be made of the asynchronous support, asynchronously issued **RECEIVE_AND_WAIT** verbs have been altered to act like [RECEIVE_AND_POST](../HIS2010/receive-and-post2.md) verbs. Specifically, while an asynchronous **RECEIVE_AND_WAIT** is outstanding, the following verbs can be issued on the same conversation:  
...
## Return Codes  
 ...
 AP_DEALLOC_NORMAL  
 Primary return code; the partner TP has deallocated the conversation without requesting confirmation and issued [DEALLOCATE](../HIS2010/deallocate1.md) with **dealloc_type** set to one of the following:  
...
1.  The local TP issues a receive verb until it finishes receiving a complete unit of data. The data received can be:  
    ...
    -   A buffer of data received independent of its logical-record format.  
     ...
     The receive verbs are [RECEIVE_AND_POST](../HIS2010/receive-and-post2.md), **RECEIVE_AND_WAIT**, and [RECEIVE_IMMEDIATE](../HIS2010/receive-immediate2.md).  
 ...
 **RECEIVE_AND_WAIT** waits for data or an indicator to be sent by the partner TP. If you need the local TP to operate continuously, use [RECEIVE_IMMEDIATE](../HIS2010/receive-immediate2.md) instead.